...
export default function RedefinirSenha() {
    ...
    const tokenRecuperacaoSenha = searchParams.get("token");
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        window.alert(confirm("Ao realizar a alteração de senha, o usuário em questão será deslogado de todos os aparelhos. Deseja continuar?"));
        if (!confirm) {
            return;
        }
        e.preventDefault();
        ...
        setMessage(null);
        try {
            await redefinirSenha({ tokenRecuperacaoSenha, novaSenha: senha });
            ...
            setMessage({ type: "error", text: "Token inválido ou expirado. Solicite uma nova recuperação." });
            setLoading(false);
        }
    };
