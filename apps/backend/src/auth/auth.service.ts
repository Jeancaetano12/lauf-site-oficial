...
export class AuthService {
  ...
  async validarSessao(refreshToken: string) {
    const sessao = await this.prisma.sessao.findFirst({
      where: { refreshToken, valido: true },
      include: { usuario: true },
    });
    ...
    if (!sessao) {
      this.logger.log(`[DEBUG] Tentativa de validar sessão com Refresh Token inválido ou revogado: ${refreshToken}`);
      throw new UnauthorizedException('Sessão inválida ou revogada.');
    }

    if (sessao.expiraEm < new Date()) {
      this.logger.log(`[DEBUG] Tentativa de validar sessão com Refresh Token expirado: ${refreshToken}`);
      await this.prisma.sessao.update({
        where: { id: sessao.id },
        data: { valido: false },
      });
    ...
    return { ok: true, usuario: userWithoutPassword };
  }
  ...
  private async gerarTokensESessao(usuarioId: string, matricula: string, cargo: string, email: string, nome: string, telefone: string, curso: string) {
    const payload = { sub: usuarioId, matricula, cargo, email, nome, telefone, curso };
    ...
    this.logger.log(`[DEBUG] Tokens gerados com sucesso para o usuario com matricula: ${matricula}, email: ${email}, cargo: ${cargo}, telefone: ${telefone}, curso: ${curso}`)
    return {
      accessToken,
      refreshToken,
