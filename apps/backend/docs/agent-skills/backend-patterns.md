...
- O service é responsável por disparar erros customizados (ex: `throw new NotFoundException('Usuario não encontrado.')`) que refletirão adequadamente na resposta para o Controller.
- **Regras de Negócio**:
  - Não é permitido criar aula em horario conflituoso com aulas ja existentes do mesmo professor, de modo geral as aulas só podem ser agendadas com 2 horas de diferença uma da outra, para mais ou para menos.
  - Não é permitido deletar ou editar uma aula que ja foi concluida ou cancelada.
  - Uma aula agendada só pode ser alterada para status CANCELADA ou CONCLUIDA. Para uma aula ser concluida, o processo de registro de presença por QR Code deve ter sido finalizado manualmente ou por limite de tempo.
